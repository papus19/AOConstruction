...
def show_sidebar_offres(user: dict) -> None:
    """
    Bloc compact dans la sidebar.
    Evite les st.expander (bug Streamlit sidebar + boutons).
    Utilise un flag _reprendre_offre_id détecté au début du cycle.
    """
    # ── Détecter si un bouton ▶ a été cliqué au cycle précédent ───
    offre_id_a_reprendre = st.session_state.pop("_reprendre_offre_id", None)
    if offre_id_a_reprendre:
        # Charger l'offre depuis Supabase et la reprendre
        try:
            database.apply_supabase_auth()
            res = database.supabase.table("offres").select("*").eq("id", offre_id_a_reprendre).execute()
            if res.data:
                _reprendre_offre(res.data[0])
        except Exception:
            pass
        # Pas de rerun ici — la navigation est déjà en session_state,
        # le prochain cycle naturel de Streamlit l'appliquera.
    ...
    # Affichage groupé SANS st.expander (bug Streamlit sidebar)
    for groupe_id, groupe_cfg in GROUPES.items():
        ...
        st.markdown(
            ...
            f"letter-spacing:.5px;margin:8px 0 4px;padding-left:2px;'>"
            f"{groupe_cfg['label']} ({len(offres_groupe)})</div>",
            unsafe_allow_html=True,
        )

        for o in offres_groupe[:4]:
            inf       = _infos(o)
            statut    = o.get("statut", "brouillon")
            cfg       = STATUTS.get(statut, {"emoji": "•", "color": "#aaa"})
            nom_court = inf["nom"][:24] + ("…" if len(inf["nom"]) > 24 else "")
            montant   = f" · {inf['total_ttc']:,.0f}$" if inf["total_ttc"] else ""

            col_info, col_btn = st.columns([4, 1])
            col_info.markdown(
                f"<div style='font-size:11px;line-height:1.4;padding:2px 0;'>"
                f"{cfg['emoji']} <b>{nom_court}</b><br>"
                f"<span style='color:#999;font-size:10px;'>{inf['date_maj']}{montant}</span>"
                f"</div>",
                unsafe_allow_html=True,
            )
            # Stocker l'ID dans session_state puis laisser Streamlit rerouler
            if col_btn.button("▶", key=f"sb_{o['id']}", help="Reprendre cette offre"):
                st.session_state["_reprendre_offre_id"] = o["id"]
                st.rerun()
